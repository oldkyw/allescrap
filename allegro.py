...
def getAuctionData(sockets=['1151', '1150'], price=(0,150), mobo=False):
    import requests
    ...
    with requests.Session() as session:
        ...
        priceCode = lambda x: '&price.from={0}&price.to={1}'.format(*x)      
        socketCode = ''
        ...
        if mobo:
            URI = 'https://api.allegro.pl/offers/listing?category.id=4228&dodatkowe-elementy=procesor&obslugiwane-procesory=Intel'+conditionCode+priceCode(price)+socketCode
        else:
            URI = 'https://api.allegro.pl/offers/listing?category.id=257222'+conditionCode+priceCode(price)+socketCode
...
if __name__ == '__main__':
    ...
    auctionList = getAuctionData(sockets=used_sockets, price=price_range, mobo=False)
